...
- GPU-инференс (CUDA) для быстрой генерации
- Параллельная генерация — несколько запросов одновременно
...
- **Docker** + **NVIDIA Container Toolkit** — контейнеризация с GPU
...
### 3. Установка NVIDIA Container Toolkit (для GPU)
...
docker build --network host -t clon_bot:1 .
...
| `NUM_WORKERS` | Нет | Количество параллельных воркеров генерации (по умолчанию `2`) |
...
docker build --network host -t clon_bot:2 .

# Откат на старую версию
docker run -d --name voice-clone-bot --gpus all --network host --env-file .env clon_bot:1 python bot.py
```
